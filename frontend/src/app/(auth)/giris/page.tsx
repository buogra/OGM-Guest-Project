...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const result = await postData<{ token: string; role: 'GUEST' | 'STAFF' | 'ADMIN' }>('/auth/login', { email, password });
      localStorage.setItem('ogm_token', result.token); localStorage.setItem('ogm_role', result.role);
      router.push(result.role === 'GUEST' ? targetUrl : '/oda-yonetimi');
              ...
              <div className="bg-white rounded-xl shadow-[0_8px_30px_rgba(0,0,0,0.08)] overflow-hidden border border-[#eaeaea]">
                ...
                <div className="p-7">
                  <form onSubmit={handleSubmit} noValidate>
                    {hata && <p className="mb-4 rounded-md border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-700">{hata}</p>}
                    <div className="mb-4.5">
                      <label className="block text-[0.83rem] font-semibold text-[#444] mb-1.5">E-posta Adresi</label>
                      <input required value={email} onChange={(e) => setEmail(e.target.value)} type="email" placeholder="ornek@ogm.gov.tr" className="w-full p-3 px-4 border-[1.5px] border-[#e0e0e0] rounded-lg text-[0.9rem] text-[#222] bg-[#fafafa] outline-none transition-all focus:border-[#2b5e39] focus:shadow-[0_0_0_3px_rgba(43,94,57,0.1)] focus:bg-white" />
                    </div>

                    <div className="mb-4.5">
                      <label className="block text-[0.83rem] font-semibold text-[#444] mb-1.5">Şifre</label>
                      <div className="relative">
                        <input required value={password} onChange={(e) => setPassword(e.target.value)} type={showPassword ? "text" : "password"} placeholder="Şifreniz" className="w-full p-3 px-4 pr-11 border-[1.5px] border-[#e0e0e0] rounded-lg text-[0.9rem] text-[#222] bg-[#fafafa] outline-none transition-all focus:border-[#2b5e39] focus:shadow-[0_0_0_3px_rgba(43,94,57,0.1)] focus:bg-white" />
