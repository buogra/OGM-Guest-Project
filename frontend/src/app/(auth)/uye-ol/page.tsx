...
export default function RegisterPage() {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const values = Array.from(event.currentTarget.querySelectorAll<HTMLInputElement>('input')).map((input) => input.value);
    const [firstName, lastName, identityNumber, registryNumber, email, phone, password, confirmPassword] = values;
    if (password !== confirmPassword) { setHata('Şifreler eşleşmiyor.'); return; }
    try { await postData('/auth/register/staff', { firstName, lastName, identityNumber, registryNumber, email, phone, password }); router.push('/giris'); }
