...
import { apiError, getData, mapRequest, patchData } from "@/lib/api";
...
export default function BekleyenTalepler() {
  ...
  useEffect(() => { void (async () => { try { setTalepler((await getData<any[]>("/admin/requests")).map(mapRequest)); } catch (error) { setHata(apiError(error)); } })(); }, []);

  const islemYap = async (id: string, yeniDurum: "Onaylandı" | "Reddedildi") => {
    try { const result = mapRequest(await patchData<any>(`/admin/requests/${id}/${yeniDurum === "Onaylandı" ? "approve" : "reject"}`)); setTalepler((prev) => prev.map((t) => t.id === id ? result : t)); }
