...
import { apiError, backendRoomStatus, getData, mapRoom, patchData } from "@/lib/api";
...
export default function OdaYonetimi() {
  ...
  useEffect(() => { void (async () => { try { setOdalar((await getData<any[]>("/admin/rooms")).map(mapRoom)); } catch (error) { setHata(apiError(error)); } })(); }, []);

  const manuelGuncelle = async () => {
    if (!secilenOda) return;
    try { const room = mapRoom(await patchData<any>(`/admin/rooms/${secilenOda.no}/status`, { manuelDurum: backendRoomStatus[manuelSecim] })); setOdalar((prev) => prev.map((o) => o.no === room.no ? room : o)); setSecilenOda(room); }
    catch (error) { setHata(apiError(error)); }
  };

  const durumSifirla = async () => {
    if (!secilenOda) return;
    try { const room = mapRoom(await patchData<any>(`/admin/rooms/${secilenOda.no}/status`, { manuelDurum: null })); setOdalar((prev) => prev.map((o) => o.no === room.no ? room : o)); setSecilenOda(room); setManuelSecim(room.otomatikDurum); }
    catch (error) { setHata(apiError(error)); }
  };
