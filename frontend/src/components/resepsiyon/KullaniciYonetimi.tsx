...
import { apiError, backendRole, deleteData, getData, mapUser, patchData, postData, putData } from "@/lib/api";
...
export default function KullaniciYonetimi() {
  ...
  const yukle = async () => {
    try { setKullanicilar((await getData<any[]>("/admin/users")).map(mapUser)); }
    catch (error) { setHata(apiError(error)); }
  };
  useEffect(() => { void yukle(); }, []);

  const aktivasyonDegistir = async (id: string) => {
    try { const user = mapUser(await patchData<any>(`/admin/users/${id}/toggle-active`)); setKullanicilar((prev) => prev.map((k) => k.id === id ? user : k)); }
    catch (error) { setHata(apiError(error)); }
  };
  ...
  const kaydet = async (e: React.FormEvent) => {
    e.preventDefault();
    const [firstName, ...last] = form.adSoyad.trim().split(/\s+/);
    const payload = { firstName, lastName: last.join(" ") || firstName, phone: "-", identityNumber: form.tcSicilNo, registryNumber: null, role: backendRole[form.rol], ...(form.password ? { password: form.password } : {}) };
    try {
      if (duzenlemeModu) {
        const user = mapUser(await putData<any>(`/admin/users/${duzenlemeModu.id}`, payload));
        setKullanicilar((prev) => prev.map((k) => k.id === user.id ? user : k));
      } else {
        if (!form.password) { setHata("Yeni kullanıcı için şifre zorunludur."); return; }
        const user = mapUser(await postData<any>("/admin/users", { ...payload, email: form.eposta, password: form.password }));
        setKullanicilar((prev) => [user, ...prev]);
      }
      setPanelAcik(false); setHata("");
    } catch (error) { setHata(apiError(error)); }
  };

  // Sil
  const sil = async (id: string) => {
    try { await deleteData(`/admin/users/${id}`); setKullanicilar((prev) => prev.filter((k) => k.id !== id)); setSilOnay(null); }
    catch (error) { setHata(apiError(error)); }
  };
        ...
        <div className="fixed inset-0 z-40 flex">
          ...
          <div className="w-80 bg-white shadow-2xl flex flex-col h-full overflow-y-auto">
            ...
            <form onSubmit={kaydet} className="flex-1 p-5 space-y-4">
              ...
              <div>
                <label className="block text-xs font-semibold text-gray-600 mb-1">{duzenlemeModu ? "Yeni Şifre (isteğe bağlı)" : "Şifre"}</label>
                <input required={!duzenlemeModu} type="password" value={form.password} onChange={(e) => setForm((f) => ({ ...f, password: e.target.value }))} className="w-full rounded-lg border border-gray-300 px-3 py-2.5 text-base focus:border-ogm-500 focus:outline-none" />
              </div>
