...
import { apiError, backendReservationStatus, deleteData, getData, mapReservation, mapRoom, patchData, postData } from "@/lib/api";
...
export default function RezervasyonYonetimi() {
  ...
  const yukle = async () => {
    try { const [rezler, roomList] = await Promise.all([getData<any[]>("/admin/reservations"), getData<any[]>("/admin/rooms")]); setRezervasyonlar(rezler.map(mapReservation)); setOdalar(roomList.map(mapRoom)); }
  ...
  const kaydet = async (e: React.FormEvent) => {
    ...
    try {
      if (duzenlemeModu) {
        const rez = mapReservation(await patchData<any>(`/admin/reservations/${duzenlemeModu.id}/status`, { durum: backendReservationStatus[form.durum] }));
        setRezervasyonlar((prev) => prev.map((r) => r.id === rez.id ? rez : r));
      } else {
        const rez = mapReservation(await postData<any>("/admin/reservations", { misafirAdi: form.misafirAdi, odaNo: Number(form.odaNo), giris: form.giris, cikis: form.cikis, durum: backendReservationStatus[form.durum] }));
        setRezervasyonlar((prev) => [rez, ...prev]);
      }
  ...
  const sil = async (id: string) => {
    try { await deleteData(`/admin/reservations/${id}`); setRezervasyonlar((prev) => prev.filter((r) => r.id !== id)); setSilOnay(null); }
    catch (error) { setHata(apiError(error)); }
  };
